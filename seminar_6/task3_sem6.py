...
with open('equation_1.txt', 'r') as f:
    pol = f.read().split()[:-2]
pol = [i for i in pol if i != '+']

with open('equation_2.txt', 'r') as f:
    pol1 = f.read().split()[:-2]
pol1 = [j for j in pol1 if j != '+']

lst = [(int(*(str(i).split('*')[:1]))) for i in pol]
lst_1 = [(int(*(str(j).split('*')[:1]))) for j in pol1]
sum_coe = list(map(sum, zip(lst, lst_1)))
a = list('*'.join(str(pol1).split()).split('*')[1:-1])
lst_2 = [('*' + a[y][:-2]) for y in range(len(a)) if y % 2 == 0]
lst_3 = [(str(v) + str(w)) for v, w in itertools.zip_longest(sum_coe, lst_2, fillvalue='')]
